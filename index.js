...
menuArray.forEach(function(item){
     ...
     <div class="menuItem"> 
            ...
            <div>
                ...
                <p><button class="button button5" id="minus-button-${item.id}" data-minus="${item.id}">-</button></p>
            </div>
        </div>
     `
...
document.addEventListener('click', function(e){
    ...
    if(e.target.dataset.minus){
       minusOrder(e.target.dataset.minus*1) 
    } 
...
function minusOrder(idOrder){
    const targetOrder = menuArray.filter(function(order){
    return order.id === idOrder
    })[0]
    let whichButton= "minus-button-"+idOrder 
    if(targetOrder.homMany>0) {
        targetOrder.homMany -=1
    } else {
        targetOrder.homMany =0
    }
    render()
}